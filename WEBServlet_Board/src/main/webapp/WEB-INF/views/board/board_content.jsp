...
<body>
    ...
    <!-- requestScope : 생략가능하지만.. 코드 가독성을 위해서 어느객체에 저장한건지 명시함 -->
    <c:set var="content" value="${requestScope.content}"></c:set>
    <c:set var="board" value="${requestScope.content.board}"></c:set>
			...
			<center>
				...
				<table width="80%" border="1">
					...
					<tr>
						...
						<a href="board_list.do?cp=${content.cpage}&ps=${content.pagesize}">목록가기</a> |
						<a href="board_edit.do?idx=${content.idx}&cp=${content.cpage}&ps=${content.pagesize}">편집</a>	|
						<a href="board_delete.do?idx=${content.idx}&cp=${content.cpage}&ps=${content.pagesize}">삭제</a> |
						<a href="board_rewrite.do?idx=${content.idx}&cp=${content.cpage}&ps=${content.pagesize}&subject=${board.subject}">답글</a>
						</td>
					</tr>
				</table>
				<!--  꼬리글 달기 테이블 -->
				...
				<br>
				<!-- 꼬리글 목록 테이블 -->
                <c:if test="${content.replylist != null && content.replylist.size() > 0}">
                    <table width="80%" border="1">
                        <tr>
